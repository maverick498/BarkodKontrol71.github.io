...
async function handleFileUpload(event) {
    ...
    try {
        ...
        for (let i = 1; i < jsonData.length; i++) {
            ...
            // İlk sütun: Barkod (9 haneli sayı)
...
function onScanSuccess(decodedText, decodedResult) {
    ...
    // 9 haneli kontrol - GS1 parse'dan sonra
    const barcode = parsedBarcode.trim();
    
    // 9 haneli değilse ama içinde 9 haneli numara varsa çıkar
    let validBarcode = barcode;
    if (barcode.length !== 9 || !/^\d+$/.test(barcode)) {
        // İçinde 9 haneli rakam ara
        const nineDigitMatch = barcode.match(/\b(\d{9})\b/);
        if (nineDigitMatch) {
            validBarcode = nineDigitMatch[1];
        } else {
            // 9 haneli bulunamadı, sessizce atla
            console.log('Geçersiz format (9 haneli değil):', barcode);
            return;
        }
    }
